...
class Cookie
{
	...
	public static function delete($name, $prefix = null)
	{
		...
		if ($config['setcookie']) {
			$response = CoroutineContent::get('response');
			if( $response instanceof \swoole_http_response){
				$response->cookie($name,'',$_SERVER['REQUEST_TIME'] - 3600,$config['path'],$config['domain'],$config['secure'],$config['httponly']);
			}
		}
		// 删除指定 cookie
		unset($_COOKIE[$name]);
			...
			foreach ($_COOKIE as $key => $val) {
				if (0 === strpos($key, $prefix)) {
					if ($config['setcookie']) {
						$response = CoroutineContent::get('response');
						if( $response instanceof \swoole_http_response){
							$response->cookie($key,'',$_SERVER['REQUEST_TIME'] - 3600,$config['path'],$config['domain'],$config['secure'],$config['httponly']);
						}
					}
					unset($_COOKIE[$key]);
				}
