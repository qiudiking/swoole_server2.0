...
//==============================================pathInfo待确认=========================
	...
	public function header($name = '', $default = null)
	{
		if (empty($this->header)) {
			$header = [];
			if (CoroutineContent::get('request') instanceof \swoole_http_request && $result = CoroutineContent::get('request')->header ) {
				$header = $result;
			} else {
	...
	public function isSsl()
	{
		...
		if (isset($server['HTTPS']) && ('1' == $server['HTTPS'] || 'on' == strtolower($server['HTTPS']))) {
			...
			return true;
		} elseif (isset($server['X-SERVER_PORT']) && ('443' == $server['X-SERVER_PORT'])) {
			return true;
